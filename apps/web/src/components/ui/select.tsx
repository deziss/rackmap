...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-9 w-full items-center justify-between whitespace-nowrap rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-foreground shadow-sm ring-offset-background",
      "placeholder:text-muted-foreground",
      "focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/60",
      "hover:bg-white/10 hover:border-white/20",
      "disabled:cursor-not-allowed disabled:opacity-50",
      "[&>span]:line-clamp-1 transition-all duration-150",
      className,
    )}
    {...props}
  >
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-xl border border-white/10 bg-card/90 backdrop-blur-xl text-foreground shadow-2xl shadow-black/30",
