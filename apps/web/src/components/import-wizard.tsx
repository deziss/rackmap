...
const KNOWN_COLUMNS = [
  ...
  { key: "sshPort", label: "SSH Port", required: false },
  { key: "domain", label: "Domain", required: false },
  { key: "environment", label: "Environment", required: false },
  { key: "cloudProvider", label: "Cloud Provider", required: false },
  { key: "gpuType", label: "GPU Type", required: false },
  { key: "gpuCount", label: "GPU Count", required: false },
  { key: "allocatedTo", label: "Allocated To", required: false },
  { key: "location", label: "Location", required: false },
  { key: "serverType", label: "Server Type", required: false },
...
export function ImportWizard({ onImported }: ImportWizardProps) {
  ...
  async function runDry() {
    ...
    try {
      ...
      const res = await fetch("/api/v1/servers/import", { method: "POST", body: fd });
  ...
  async function commit() {
    ...
    try {
      ...
      const res = await fetch("/api/v1/servers/import", { method: "POST", body: fd });
      const data = await res.json() as ImportResult;
      toast.success(`Imported ${data.created} servers`);
      setStep("done");
      onImported();
    } catch (e: unknown) {
      toast.error((e as Error).message);
      ...
      <DialogContent className="max-w-2xl">
        ...
        {step === "map" && (
          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">Map your file columns to server fields. Required: hostname + ip.</p>
            ...
            <div className="flex justify-between">
              ...
              <Button size="sm" onClick={runDry} disabled={loading || !mapping.hostname || !mapping.ip}>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="max-h-52 overflow-y-auto rounded border divide-y divide-border text-xs font-mono">
              {dryResult.errors.map((r) => (
                <div key={r.row} className={`px-3 py-1.5 flex items-center gap-2 ${r.status === "error" ? "bg-red-50 dark:bg-red-950/20" : r.status === "skip" ? "bg-yellow-50 dark:bg-yellow-950/20" : ""}`}>
                  ...
                  <span className="flex-1 truncate">{r.hostname}</span>
            ...
            <div className="flex justify-between">
              ...
              <Button size="sm" onClick={commit} disabled={loading || dryResult.created === 0}>
                {loading ? "Importing…" : `Import ${dryResult.created} servers`}
              </Button>
            </div>
          </div>
        )}
